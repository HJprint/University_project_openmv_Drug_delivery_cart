...
def num_zuo_you(num_cx):
    red_cx=160
    #以底下中间的红色为基准识别
    red_dis = img.find_blobs([red_lab2],roi = (0,190,320,40), pixels_threshold=20, area_threshold=20,merge=True)
    if red_dis:
        red_max=find_max(red_dis)
        red_cx=red_max.cx()
    if num_cx<red_cx:return 9#数字在左边
...
while(1):
    ...
    red_cross_ding = img.find_blobs([black_lab],roi = (0,20,320,100), pixels_threshold=20, area_threshold=20,merge=True)
    ...
    img.draw_rectangle((0,20,320,100),color=(0,225,0),thickness=2)#识别红色右边的限制框
    ...
    #第三个路口判断
    if red_cross_ding:
        red_m=find_max(red_cross_ding)
        img.draw_rectangle(red_m.rect(),color=(0,0,225),thickness=2)#识别红色右边的限制框
        #print(red_m.h()/red_m.w())
        if red_m.h()/red_m.w()<0.3:
            t_5=1
            print("T字")
        else:
            t_5=0
    ...
    if red_cross:
        for blob1 in red_cross:
            ...
            if(((red_cross_l and red_cross_z )or (red_cross_r and red_cross_z) or (red_cross_r and red_cross_l) and data_car==0)):#判断是否拐弯
                ...
                if num1!=1 and num1!=2:
                    ...
                    for x in range(1):
                        if num1==num_datas[0] or num1==num_datas[2] or num1==num_datas[4] and zhuan==0 and data_car==0:#判断是不是
                ...
                for x in range(10):
                    ...
                    uart.write("%d"%(660))
                    print('前')
            elif (red_cross_d_l and red_cross_z )or (red_cross_d_r and red_cross_z) or (red_cross_d_r and red_cross_d_l) or t_5==1 and data_car!=1:#判断是否拐弯
                ...
                if num1!=1 and num1!=2  and zhuan!=0:
                    if data_car==2:zhuan=900
                    if zhuan == 999:
                        if t_5 == 1:zhuan = 900;
                        else:zhuan = 160;
